...
def iv_dv_get_col_names(data,target):
    ...
    for i in data.columns:
        #if i == target:
        #    continue
        if data.loc[:,i].dtype is not str and len(data.loc[:,i].value_counts()) <30:
            print('Need to define this col - %s in categ variable'%i)
...
def get_outliers(data,continous_names, outliers_,except_ = [],drop=False):
    ...
    for i in continous_names:
        ...
        outliers_.append('per outliers for %s is %s'%(i, ( (len(data[data.loc[:,i] > max])/len(data)) )*100 ))
        outliers_.append('per outliers for %s is %s'%(i, ( (len(data[data.loc[:,i] < min])/len(data)) )*100 ))
